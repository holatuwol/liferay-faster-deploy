...
def get_jira_cookie():
	...
	while jira_cookie is None:
		try:
			jira_username = subprocess.call(['git', 'config', '--global', 'jira.session-username', jira_cookie_name])
			jira_password = subprocess.call(['git', 'config', '--global', 'jira.session-password', jira_cookie_value])
		except:
