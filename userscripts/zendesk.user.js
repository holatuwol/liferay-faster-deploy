...
function getPropertyBoxes(ticketId) {
    var propertyBoxes = Array.from(document.querySelectorAll('#ticket_sidebar div[data-garden-id="grid.row"]'));
    var visiblePropertyBoxes;
    if (propertyBoxes.length == 0) {
        propertyBoxes = Array.from(document.querySelectorAll('.property_box:not(.ticket_properties)'));
        visiblePropertyBoxes = propertyBoxes.filter(function (it) {
        ...
        visiblePropertyBoxes = propertyBoxes.filter(function (it) {
            var workspaceElement = it.closest('.workspace');
            return workspaceElement && workspaceElement.style.display != 'none';
        }).filter(function (it) { return it.querySelector('div[data-test-id="ticket-fields-tags"]') == null; });
    }
    if (ticketId) {
...
function closeAllTabs() {
    var tablist = document.querySelector('[data-test-id="header-tablist"]:not(.lesa-ui-close-all-tabs)');
    if (!tablist) {
        return;
    }
    ...
    var updateVisibleTabCount = function () {
        var visibleCloseButtons = Array.from(tablist.querySelectorAll('[data-test-id="close-button"]:not(.collapsed)'));
        tablist.setAttribute('data-visible-tabs', String(visibleCloseButtons.length));
    };
    updateVisibleTabCount();
    var observer = new MutationObserver(updateVisibleTabCount);
