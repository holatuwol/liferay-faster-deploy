...
if (window.location.hostname == '24475.apps.zdusercontent.com') {
    ...
    styleElement.textContent += '[type="internal"] { transition: max-height 0.3s ease, padding 0.3s ease;  overflow: hidden;  max-height: 1000px;}[type="internal"].collapsed {  max-height: 0 !important;  padding: 0 !important;  opacity: 0;  pointer-events: none;}'
}
...
function closeAllTabs() {
    ...
    if (tablist && visibleCloseButtons.length > 0 && !document.querySelector('#close-all-tab-btn')) {
        ...
        button.addEventListener('click', () => {
            document.querySelectorAll('[data-test-id="close-button"]').forEach(btn => btn.click());
            button.remove();
        });
    }
}
